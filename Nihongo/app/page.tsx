...
export default function NihonGoLanding() {
  ...
  useEffect(() => {
    ...
    setVisitCount(Math.floor(Math.random() * 1000) + 500)
  ...
  return (
    <div className="min-h-screen relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-10">
        ...
        {/* Bottom Left Branch */}
        <div className="absolute bottom-0 left-0 w-48 h-48 opacity-25 transform rotate-180">
        ...
        {/* Bottom Right Branch */}
        <div className="absolute bottom-0 right-0 w-48 h-48 opacity-25 transform rotate-180 scale-x-[-1]">
          <svg viewBox="0 0 150 150" className="w-full h-full">
            <path d="M10,10 Q25,30 40,25 Q55,20 70,35 Q85,50 100,40" stroke="#8B4513" strokeWidth="3" fill="none" />
            <circle cx="40" cy="25" r="6" fill="#FFB6C1" opacity="0.7" />
            <circle cx="70" cy="35" r="5" fill="#FFC0CB" opacity="0.8" />
        ...
        {/* Side Branches */}
        <div className="absolute left-0 top-1/2 w-32 h-32 opacity-20 transform -translate-y-1/2">
        ...
        <div className="absolute right-0 top-1/2 w-32 h-32 opacity-20 transform -translate-y-1/2 scale-x-[-1]">
          <svg viewBox="0 0 100 100" className="w-full h-full">
            <path d="M0,50 Q20,30 40,50 Q60,70 80,50" stroke="#8B4513" strokeWidth="2" fill="none" />
            <circle cx="40" cy="50" r="4" fill="#FFB6C1" opacity="0.8" />
            <circle cx="80" cy="50" r="3" fill="#FFC0CB" opacity="0.7" />
      ...
      <div className="relative z-30 max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col items-center justify-center min-h-screen text-center mb-16">
          <h1 className="text-6xl md:text-8xl font-black bg-gradient-to-r from-pink-600 via-rose-500 to-pink-400 bg-clip-text text-transparent mb-4 relative">
            Nihon-GO!
            <span className="absolute -top-8 -right-8 text-2xl opacity-70 animate-bounce">🌸</span>
          </h1>
          <p className="text-xl md:text-2xl text-purple-700 font-light tracking-wide opacity-0 animate-fade-in-up">
            Explore Japan in Three Different Era • A Japanese Learning Adventure
          </p>
        </div>

        {/* GitHub Section */}
        <div className="bg-white/90 backdrop-blur-xl border-2 border-pink-200 rounded-3xl p-8 mb-8 text-center shadow-2xl">
          <button
            onClick={handleGitHubClick}
            className="inline-flex items-center gap-3 text-purple-700 hover:text-pink-600 font-medium text-lg transition-all duration-300 px-8 py-4 border-2 border-purple-200 hover:border-pink-300 rounded-full bg-white/50 hover:bg-white/80 hover:-translate-y-1 hover:shadow-lg"
          >
          ...
          <div className="mt-8 p-6 bg-white/60 rounded-2xl border border-pink-200 relative">
            <div className="text-4xl font-bold text-pink-600 mb-2 animate-pulse">{visitCount.toLocaleString()}</div>
            <div className="text-purple-600 font-medium">GitHub Visits</div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-8">
          {/* Slideshow Panel */}
          <div className="lg:col-span-2 bg-white/90 backdrop-blur-xl border-2 border-pink-200 rounded-3xl p-8 shadow-2xl">
            <h2 className="text-2xl font-bold text-pink-600 mb-6 flex items-center gap-2">🖼️ Game Screenshots</h2>
            ...
            <div className="flex justify-center gap-4 mb-4">
              <button
                onClick={prevSlide}
                className="px-6 py-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-full hover:shadow-lg transition-all duration-300 hover:-translate-y-1"
              >
                ← Previous
              </button>
              <button
          ...
          {/* Video Panel */}
          <div className="bg-white/90 backdrop-blur-xl border-2 border-pink-200 rounded-3xl p-8 shadow-2xl">
            <h2 className="text-2xl font-bold text-pink-600 mb-6 flex items-center gap-2">🎥 Game Trailer</h2>
            <div className="relative h-64 rounded-2xl overflow-hidden shadow-xl mb-4">
              <iframe
                width="100%"
                height="100%"
          ...
          {/* Description Panel */}
          <div className="bg-white/90 backdrop-blur-xl border-2 border-pink-200 rounded-3xl p-8 shadow-2xl">
            <h2 className="text-2xl font-bold text-pink-600 mb-6 flex items-center gap-2">📖 About the Game</h2>
        ...
        {/* Developers Section */}
        <div className="bg-white/90 backdrop-blur-xl border-2 border-pink-200 rounded-3xl p-8 shadow-2xl">
          <h2 className="text-2xl font-bold text-pink-600 mb-8 text-center flex items-center justify-center gap-2">
            👥 Meet the Developers
          </h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {developers.map((dev, index) => (
              <div
                key={index}
                className="bg-white/80 backdrop-blur-sm border border-pink-200 rounded-2xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2 group"
              >
                ...
                <h3 className="text-lg font-bold text-pink-600 mb-1">{dev.name}</h3>
                <p className="text-sm font-semibold text-purple-600 uppercase tracking-wide mb-3">{dev.role}</p>
                <p className="text-xs text-gray-600 leading-relaxed mb-4">{dev.description}</p>
